...
async function generateMiniMaxImage(baseUrl, upstream, request, options) {
	const model = wireModel(request);
	if (request.prompt.length >= MINIMAX_MAX_PROMPT_LENGTH) throw new ImageGenError(`MiniMax image-01 要求提示词少于 ${MINIMAX_MAX_PROMPT_LENGTH} 字符（当前 ${request.prompt.length}），请精简后重试`, "prompt-too-long");
	const body = {
		model,
		prompt: request.prompt,
